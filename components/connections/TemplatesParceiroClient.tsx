...
export function TemplatesParceiroClient() {
  ...
  const acao = useMutation({
    ...
    onError: (e: unknown) =>
      toast.error(e instanceof Error ? e.message : t("Não consegui falar com a plataforma.")),
  });
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              <option value="MARKETING">
                {t("Marketing — promoção, novidade, reengajamento")}
              </option>
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      ...
                      if (!r.ok || !j.data?.url) {
                        ...
                        toast.error(j.error?.message ?? t("Não consegui subir a imagem."));
                        return;
                      }
              ...
              {botoes.map((b, i) => (
                <div key={i} className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    ...
                    <option value="phone_number">{t("Ligar")}</option>
                  </select>
                  <input
                    value={b.texto}
                    onChange={(e) => {
      ...
      {lista.isLoading ? (
        ...
        <p className="text-sm text-muted-foreground">
          {t("Nenhum modelo espelhado ainda.")} {t("Clique em")} <strong>{t("Sincronizar")}</strong>{" "}
        ...
        <ul className="divide-y divide-border rounded-md border border-border">
          {templates.map((template) => {
            ...
            const c = lerConteudo(template.components);
            ...
            return (
              <li key={chave} className="px-3 py-2">
                ...
                >
                  ...
                  <span
                    className={cn(
                      ...
                      COR_DO_ESTADO[template.status?.toUpperCase()] ?? "text-muted-foreground",
                    )}
                  >
                    {template.status}
                  </span>
                ...
                {template.rejectedReason && (
                  <p className="mt-1 text-[11px] text-destructive">{template.rejectedReason}</p>
                )}

                {expandido && (
                  <div className="bg-muted/40 mt-2 flex flex-col gap-1.5 rounded-md p-2 text-sm">
                    {c.header && (
                      <p className="text-xs">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {t("Sincronizado em")} {new Date(template.syncedAt).toLocaleString("pt-BR")}
                    </p>
                  </div>
                )}
              </li>
