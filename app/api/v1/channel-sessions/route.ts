...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const { data: created, error: insErr } = await supabase
    ...
    .insert({
      ...
      ...sessionReferenceColumns(parsed.data.provider, sessionName),
  ...
  try {
    await provisionChannel(parsed.data.provider, {
      organizationId: activeOrg.orgId,
      sessionRef: sessionName,
      displayName: parsed.data.display_name ?? null,
    });
  } catch (err) {
    // Rollback : aucun canal fantôme ne doit rester en STARTING si le transport
    // n’a pas pu créer l’instance.
    await supabase
      .from("channel_sessions")
      .delete()
      .eq("organization_id", activeOrg.orgId)
      .eq("id", created.id);
    return fail(provisionErrorCode(parsed.data.provider), friendlyProvisionError(err), 502, { requestId });
  }
  ...
  void audit({
    ...
    requestId,
    metadata: { provider: parsed.data.provider, session_ref: sessionName },
  });
