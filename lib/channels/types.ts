...
export type ChannelProvider = "waha" | "meta_cloud" | "zernio" | "evolution_go";
...
export interface ChannelAdapter {
  ...
  /** Retourne le QR courant sous forme de bytes, sans exposer la clé transport. */
  getPairingQr?(input: ChannelTenantScope & { sessionRef: string }): Promise<{
    bytes: ArrayBuffer;
    contentType: string;
  }>;

  reconnect?(input: ChannelTenantScope & { sessionRef: string; force: boolean }): Promise<{ status: string | null }>;
  deleteSession?(input: ChannelTenantScope & { sessionRef: string }): Promise<void>;
