...
export class WahaClient {
  ...
  async getPairingQrImage(name: string): Promise<{ bytes: ArrayBuffer; contentType: string }> {
    const res = await fetch(`${this.baseUrl}/api/${encodeURIComponent(name)}/auth/qr?format=image`, {
      headers: { "X-Api-Key": this.apiKey },
      cache: "no-store",
    });
    if (!res.ok) throw new Error(`waha_${res.status}`);
